package data

import (
	"github.com/pegasus-toolset/color"
	"github.com/pegasus-toolset/color/palette"
)

var Palettes = map[string]color.Palette{
	"microsoft windows default 16-color palette": palette.Windows16,
	"windows16": palette.Windows16,

	"microsoft windows default 20-color palette": palette.Windows20,
	"windows20": palette.Windows20,

	"apple macintosh default 16-color palette": palette.Macintosh,
	"macintosh": palette.Macintosh,

	"risc os default palette": palette.RISC,
	"risc":                    palette.RISC,

	"bluecurve icon palette": palette.Bluecurve,
	"bluecurve":              palette.Bluecurve,

	"eclipse-style palette": palette.Eclipse,
	"eclipse":               palette.Eclipse,

	"tango desktop project palette": palette.Tango,
	"tango":                         palette.Tango,

	"apollo": palette.Apollo,

	"aap-16": palette.AAP16,
	"aap16":  palette.AAP16,

	"aap-64": palette.AAP64,
	"aap64":  palette.AAP64,

	"aap-majestyxvii": palette.AAPMajestyXVII,
	"aapmajestyxvii":  palette.AAPMajestyXVII,

	"aap-micro12": palette.AAPMicro12,
	"aapmicro12":  palette.AAPMicro12,

	"aap-radiantxv": palette.AAPRadiantXV,
	"aapradiantxv":  palette.AAPRadiantXV,

	"aap-splendor128": palette.AAPSplendor128,
	"aapsplendor128":  palette.AAPSplendor128,

	"petite-8": palette.Petite8,
	"petite8":  palette.Petite8,

	"petite-8 afterdark": palette.Petite8Afterdark,
	"petite8afterdark":   palette.Petite8Afterdark,

	"simplejpc-16": palette.SimpleJPC16,
	"simplejpc16":  palette.SimpleJPC16,

	"blk 36": palette.BLK36,
	"blk36":  palette.BLK36,

	"blk aqu4": palette.BLKAQU4,
	"blkaqu4":  palette.BLKAQU4,

	"nopal-12": palette.NOPAL12,
	"nopal12":  palette.NOPAL12,

	"pollen8": palette.Pollen8,

	"arq4": palette.ARQ4,

	"arq16": palette.ARQ16,

	"edg77": palette.EDG77,

	"en4": palette.EN4,

	"endesga 8": palette.Endesga8,
	"endesga8":  palette.Endesga8,

	"endesga 16": palette.Endesga16,
	"endesga16":  palette.Endesga16,

	"endesga 32": palette.Endesga32,
	"endesga32":  palette.Endesga32,

	"endesga 36": palette.Endesga36,
	"endesga36":  palette.Endesga36,

	"endesga 64": palette.Endesga64,
	"endesga64":  palette.Endesga64,

	"endesga soft 16": palette.EndesgaSoft16,
	"endesgasoft16":   palette.EndesgaSoft16,

	"enos16": palette.ENOS16,

	"hept32": palette.Hept32,

	"superfuture25": palette.SUPERFUTURE25,

	"fantasy 24": palette.Fantasy24,
	"fantasy24":  palette.Fantasy24,

	"coldfire gb": palette.ColdfireGB,
	"coldfiregb":  palette.ColdfireGB,

	"earth gb": palette.EarthGB,
	"earthgb":  palette.EarthGB,

	"ice cream gb": palette.IceCreamGB,
	"icecreamgb":   palette.IceCreamGB,

	"island joy 16": palette.IslandJoy16,
	"islandjoy16":   palette.IslandJoy16,

	"mist gb": palette.MistGB,
	"mistgb":  palette.MistGB,

	"nymph gb": palette.NymphGB,
	"nymphgb":  palette.NymphGB,

	"peachy pop 16": palette.PeachyPop16,
	"peachypop16":   palette.PeachyPop16,

	"resurrect 32": palette.Resurrect32,
	"resurrect32":  palette.Resurrect32,

	"resurrect 64": palette.Resurrect64,
	"resurrect64":  palette.Resurrect64,

	"rustic gb": palette.RusticGB,
	"rusticgb":  palette.RusticGB,

	"wish gb": palette.WishGB,
	"wishgb":  palette.WishGB,

	"kirokaze gameboy": palette.KirokazeGameboy,
	"kirokazegameboy":  palette.KirokazeGameboy,

	"na16": palette.NA16,

	"bubblegum 16": palette.Bubblegum16,
	"bubblegum16":  palette.Bubblegum16,

	"cave": palette.Cave,

	"journey": palette.Journey,

	"pear36": palette.Pear36,

	"pineapple 32": palette.Pineapple32,
	"pineapple32":  palette.Pineapple32,

	"rosy 42": palette.Rosy42,
	"rosy42":  palette.Rosy42,

	"taffy 16": palette.Taffy16,
	"taffy16":  palette.Taffy16,

	"steam lords": palette.SteamLords,
	"steamlords":  palette.SteamLords,

	"vinik24": palette.Vinik24,

	"aurora": palette.Aurora,

	"blessing": palette.Blessing,

	"blk neo": palette.BLKNEO,
	"blkneo":  palette.BLKNEO,

	"borkfest": palette.Borkfest,

	"cc-29": palette.CC29,
	"cc29":  palette.CC29,

	"chocomilk-8": palette.CHOCOMILK8,
	"chocomilk8":  palette.CHOCOMILK8,

	"dawnbringer 32": palette.DawnBringer32,
	"dawnbringer32":  palette.DawnBringer32,

	"duel": palette.Duel,

	"famicube": palette.Famicube,

	"lospec500": palette.Lospec500,

	"lux2k": palette.Lux2K,

	"nintendo entertainment system": palette.NintendoEntertainmentSystem,
	"nintendoentertainmentsystem":   palette.NintendoEntertainmentSystem,

	"nyx8": palette.Nyx8,

	"oil 6": palette.Oil6,
	"oil6":  palette.Oil6,

	"👌31":  palette.OK31,
	"ok31": palette.OK31,

	"1bit monitor glow": palette.OnebitMonitorGlow,
	"1bitmonitorglow":   palette.OnebitMonitorGlow,
	"onebitmonitorglow": palette.OnebitMonitorGlow,

	"pico-8": palette.PICO8,
	"pico8":  palette.PICO8,

	"slso8": palette.SLSO8,

	"sweetie 16": palette.Sweetie16,
	"sweetie16":  palette.Sweetie16,

	"twilight 5": palette.Twilight5,
	"twilight5":  palette.Twilight5,

	"vine's flexible linear ramps": palette.VinesFlexibleLinearRamps,
	"vinesflexiblelinearramps":     palette.VinesFlexibleLinearRamps,

	"zughy 32": palette.Zughy32,
	"zughy32":  palette.Zughy32,
}
